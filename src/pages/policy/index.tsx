...
const PolicyView: React.FC<PolicyProps> = props => {
  ...
  return (
    <PageHeaderWrapper pageHeaderRender={() => (<></>)} content='content'>

      <br />
      <div style={{ textAlign: 'center' }}>
        <h1>BOC Merchant Portal Policy</h1>
      </div>
      ...
      <br />
      <div style={{ textAlign: 'center' }}>
        <Checkbox checked={isAgree} onChange={() => setIsAgree(!isAgree)}>已仔细阅读协议</Checkbox>
        <Button type="primary" shape="round" disabled={!isAgree} onClick={handleAgree} loading={loading}>同意</Button>
      </div>
    </PageHeaderWrapper>
  )
};
